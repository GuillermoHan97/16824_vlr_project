...
def compute_confusion_matrix(df_merged, thres):
    ...
    print('\tLAM\tNLAM\nLAM\t{:.3f}\t{:.3f}\nNLAM\t{:.3f}\t{:.3f}'.format(
        ...
        FP / (FP+TN) * 100, 
        TN / (FP+TN) * 100))
    weighted_acc = (TP/(TP+FP)+TN/(TN+FN))/2
    print('weighted accuracy {:.3f}'.format(weighted_acc))
    print('precision {:.3f}'.format(TP/(TP+FP)))
    weighted_acc1 = (TP/(TP+FN)+TN/(TN+FP))/2
    print('weighted accuracy1 {:.3f}'.format(weighted_acc1))
    print('precision1 {:.3f}'.format(TP/(TP+FN)))
    print('TP',TP)
    print('FN',FN)
    print('FP',FP)
    print('TN',TN)
